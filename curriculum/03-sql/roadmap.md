...
### 핵심 추출 내용(액기스)
1. **질문→SQL 번역**: 쿼리 작성 전 질문/지표/필터를 문장으로 먼저 정의.
2. **JOIN 정확성 우선**: 성능 튜닝 전에 중복/누락/카디널리티 오류를 먼저 검증.
3. **CTE로 가독성 확보**: 복잡한 쿼리는 단계 분해가 유지보수에 유리.
4. **윈도우 함수는 분석의 핵심 도구**: 순위/누적/전후 비교를 단순화.

### 반영 매핑
- Week1: KPI 질의/집계 정확성
- Week2: JOIN/CTE 리팩터링
- Week3: 윈도우 함수 기반 리텐션/시계열 분석
